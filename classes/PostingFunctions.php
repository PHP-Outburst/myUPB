...
class PostingFunctions
{
    ...
    {
        $text = PostingFunctions::filterLanguage(PostingFunctions::format_text(PostingFunctions::UPBcoding($text)), $_CONFIG);
        $text = str_replace('&lt;x&gt;','',$text);
        $text = (get_magic_quotes_gpc()) ? stripslashes($text) : $text;

        //Do we need to remove meta data?
        if ($removeMeta)
            $text = preg_replace('/<meta (.*?)>/i', '\n', $text);
    ...
    public static function UPBcoding($text)
    {
        ...
        $msg = preg_replace('/\[size=(.*?)\](.*?)\[\/size\]/si', '<span style="font-size:\\1px;2">\\2</span>', $msg);
        ...
        while (preg_match('/\[quote(.*?)\](.*?)\[\/quote\]/si', $msg)) {
            $msg = preg_replace_callback('/\[quote(.*?)\](.*?)\[\/quote\]/si', 'PostingFunctions::parse_quote', $msg);
        }
