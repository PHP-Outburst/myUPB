...
class PostingFunctions
{
    public static function message_icons()
    {
    ...
    public static function format_text($text,$type='') {
        if ($type != 'edit')
        {
            $text = str_replace("\n", "<br />", $text);
    ...
    public static function display_msg($text,$type = '',$removeMeta=false)
    {
        $text = static::filterLanguage(static::format_text(static::UPBcoding($text)), $_CONFIG);
    ...
    public static function UPBcoding($text) {
        ...
        $msg = preg_replace("/\[youtube\](.*?)\[\/youtube\]/si", '<iframe width="425" height="344" src="https://www.youtube.com/embed/\\1" frameborder="0" allowfullscreen></iframe>', $msg);
        ...
        {
            $msg = preg_replace_callback("/\[quote(.*?)\](.*?)\[\/quote\]/si",'static::parse_quote',$msg);
        }
        //$msg = preg_replace("/\[code\](.*?)\[\/code\]/si", "<font color='red'>Code:<hr><pre>\\1<hr></pre></font>", $msg);

        $msg = preg_replace_callback("!\[code=([\w\-]+)](.*)\[/code]!Us","static::geshify", $msg);
        $msg = preg_replace_callback("!\[code\](.*)\[/code]!Us","static::geshify", $msg);
...
//finds the correct type of quote and converts to correct quote output
    public static function parse_quote($matches)
    {
    ...
    public static function bbcodebuttons($txtarea='message',$type='post') {
    ...
    public static function getSmilies($field = 'message')
    {
        $tdb = new tdb(DB_DIR.'/', 'bbcode.tdb');
        $tdb->setFP("smilies","smilies");
    ...
    public static function geshify($match) {
        ...
        $data = rtrim($data);
        //$data = str_replace("\n\n", "\n", $data);
        $_geshi = new GeSHi($data, $language);
        //$_geshi->set_header_type(GESHI_HEADER_DIV);
        if($language == "cpp")$language = "c++";
        if($language == "javascript")$language = "JavaScript";
        $_geshied = str_replace("&amp;", "&", $_geshi->parse_code());
        $_geshied = str_replace("<br />", "", $_geshied);
        $_geshied = str_replace("<br/>", "", $_geshied);
        $_geshied = str_replace("<br>", "", $_geshied);
        $_geshied = str_replace("\r", "", $_geshied);
        // geshicode_h may be used as an image header.
