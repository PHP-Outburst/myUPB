...
class ErrorHandler {
	...
	public function add_error($errno, $errstr, $errfile = '', $errline = '')
    {
		/*
		 [0]$errno = integer;
		 [1]$errstr = string;
		...
		if ($errfile != '') {
            ...
            if (defined('DB_DIR'))
                $errfile = str_replace(DB_DIR, '<DATA_DIRECTORY>', $errfile);
		}
	...
	public function print_errors($in_table = false)
    {
		reset($this->error);
		...
		if ($this->error_exists()) {
            ...
            if ($in_table)
                echo '<table class="error"><td><td class="error">';
			else
                echo '<br>';
            ...
            foreach ($this->error as $error) {
                ...
                if($error[3] != '')
                    echo ' on line <b>' . $error[3] . '</b>';

                if($error[2] != '')
                    echo ' in file <b>' . $error[2] . '</b>';
